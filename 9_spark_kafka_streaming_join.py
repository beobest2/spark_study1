...
spark = (
    SparkSession.builder.appName("StreamingJoin")
...
impressions_df = (
    impression_events.select(
        col("key"),
        from_json(col("value").cast("string"), impression_schema).alias("value"),
    )
    .select("value.*")
...
clicks_df = (
    click_events.select(
        col("key"), from_json(col("value").cast("string"), click_schema).alias("value")
    )
    .select("value.*")
    .withColumn("create_date", to_timestamp("create_date", timestamp_format))
...
join_df = (
    impressions_df.join(clicks_df, impressions_df.uuid == clicks_df.uuid, "inner")
    .drop(clicks_df.uuid)
    .drop(clicks_df.placement_id)
)

# Start running the query that prints the running counts to the console
query = join_df.writeStream.outputMode("append").format("console").start()
